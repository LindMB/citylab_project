...
void Patrol::laserscan_callback_(
  ...
  if (is_obstacle_detected_(msg)) {
    ...
    auto result_future = this->direction_service_client_->async_send_request(
        request, std::bind(&Patrol::handle_service_response_, this,
                           std::placeholders::_1));
  } else {
    // Do nothing
  }
...
void Patrol::handle_service_response_(
  ...
  if (response->direction == "right") {
    ...
    this->direction_ = 0.5;
  } else { // response->direction == "forward"
    this->direction_ = 0.0;
  }
}
...
void Patrol::avoid_obstacle_() {
  ...
  avoid_msg.angular.z = this->direction_;
...
bool Patrol::is_obstacle_detected_(
  ...
  for (int i = 0; i < (int)msg->ranges.size(); i++) {
    ...
    if (std::isfinite(msg->ranges[i]) && msg->ranges[i] < 0.35) {
      ...
      // For a ray of the front-right section (between -15deg and -30deg) of the
      // lidar...
      else if (angle >= -M_PI / 6 && angle <= -M_PI / 12) {
        RCLCPP_INFO(this->get_logger(),
...
int main(int argc, char **argv) {
  ...
  robot_patrol_node =
      std::make_shared<Patrol>("robot_patrol_with_service_node");
